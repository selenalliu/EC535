...
static ssize_t mytimer_read(struct file *filp, char *buf, size_t count, loff_t *f_pos) {
	int i;
	char tbuf[256], *tbptr = tbuf; // temporary buffer
	size_t len;

	if (*f_pos > 0) {
        return 0; // EOF, stop further reads
    }
...
static ssize_t mytimer_write(struct file *filp, const char *buf, size_t count, loff_t *f_pos) {
	...
	if (sscanf(kbuf, "s %d %127[^\n]", &sec, msg) == 2) {
		...
		// add new timer if not found in existing timers & within limit
		for (i = 0; i < max_active_timers; i++) {
			if (!timers[i].active) {
		...
		if (i == max_active_timers) {
			...
			// let user know that max active timers reached
			fprintf(stderr, "%d timer(s) already exist(s)!\n", max_active_timers);
			return count;
		}
